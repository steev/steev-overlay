...
pkg_setup() {
	...
	eerror "enable gadu gadu support in Carrier."
	eerror
	...
	if use ncurses &&  ! built_with_use sys-libs/ncurses unicode; then
		...
		eerror "to build finch the console client of Carrier."
		eerror
		die "Configure failed"
	fi
	...
	if use zephyr && ! built_with_use app-crypt/mit-krb5 krb4 ; then
		...
		eerror "enable krb4 support for the zephyr protocol in Carrier"
		eerror
		die "Configure failed"
	fi
